...
export default function MediaRow({ title, items, type }: MediaRowProps) {
  ...
  return (
    <section className="mt-8 group/row relative">
      <h2 className="text-xl font-black mb-6 text-white uppercase tracking-[0.3em] px-4 md:px-12 border-l-[6px] border-red-600 ml-4 md:ml-0 pl-4 italic leading-none">
        {title}
      </h2>
      ...
      <div 
        ref={scrollRef}
        className="flex gap-5 overflow-x-hidden pb-10 px-4 md:px-12 scroll-smooth"
      >
        {items.map((item) => (
          <Link 
            key={item.id} 
            href={`/${item.media_type || type || 'movie'}/${item.id}`}
            className="min-w-[180px] md:min-w-[260px] relative aspect-[2/3] rounded-2xl overflow-hidden group bg-neutral-900 shadow-[0_0_30px_rgba(0,0,0,0.5)] transition-all duration-500 hover:scale-[1.03] hover:shadow-red-600/20 hover:ring-2 hover:ring-red-600 ring-offset-4 ring-offset-neutral-950 block"
          >
            <Image 
