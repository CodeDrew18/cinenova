...
//           <div className="hidden lg:flex items-center gap-8 text-[11px] font-bold uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400">
//             {navLinks.map((link) => (
//               <Link
//                 key={link.href}
//                 href={link.href}
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 w-full z-[100] bg-white/70 dark:bg-neutral-950/60 backdrop-blur-2xl border-b border-black/5 dark:border-white/5 h-20">
      <div className="max-w-[1400px] mx-auto px-6 h-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-12">
          <Link
            href="/"
            className="text-3xl font-black italic tracking-[-0.05em] text-red-600"
          >
            CINE<span className="text-neutral-500 dark:text-neutral-300">NOVA</span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-8 text-[11px] font-bold uppercase tracking-[0.2em]">
            {navLinks.map((link) => (
              <Link
                ...
                href={link.href}
                className={`
                  text-neutral-400 hover:text-neutral-200 transition-colors relative group
                  ${pathname === link.href ? 'text-neutral-200' : ''}
                `}
              >
                {link.name}
                <span
                  className={`absolute -bottom-1 left-0 h-[1px] bg-red-600 transition-all ${
        ...
        <div className="flex items-center gap-6">
          ...
          <form
            ...
            className="hidden lg:flex items-center bg-neutral-100 dark:bg-neutral-900/60 rounded-full px-5 py-2 border border-black/5 dark:border-white/10"
          >
            <select
              value={searchType}
              onChange={(e) => setSearchType(e.target.value as SearchType)}
              className="bg-transparent text-[10px] font-black uppercase text-neutral-500 outline-none border-r border-neutral-300 dark:border-white/10 pr-2 mr-2"
            >
              <option value="multi">All</option>
              <option value="movie">Movies</option>
              <option value="tv">TV</option>
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="bg-transparent border-none text-[10px] font-bold w-48 placeholder:text-neutral-400 uppercase text-neutral-700 dark:text-neutral-200"
            />
          ...
          {user ? (
            <div className="flex items-center gap-4 text-neutral-400">
              <Link
                href="/profile"
                className="text-sm font-bold uppercase tracking-wide hover:text-neutral-200 transition-colors"
              >
                {user.user_metadata?.username || user.email?.split('@')[0]}
              </Link>
            ...
            <Link
              href="/login"
              className="text-[11px] font-black uppercase tracking-widest bg-red-600 px-6 py-2 hover:bg-red-700 transition-all"
            >
              Login
            </Link>
          )}
