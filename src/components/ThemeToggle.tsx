...
export default function ThemeToggle() {
  ...
  // Keep a CSS custom property for icon color so components (hamburger) can read it
  React.useEffect(() => {
    const curr = resolvedTheme ?? theme;
    const color = curr === 'dark' ? '#ffffff' : '#0f172a';
    try {
      document.documentElement.style.setProperty('--hamburger-icon-color', color);
    } catch (e) {
